...
def run_backtest(sig_data, probs, threshold=None,
    ...
    规则:
      1. 每日: 先平仓到期持仓
      2. 买入: 按gap_pct降序选股 (排序铁律), 概率<threshold的信号跳过
    ...
    for day in sig_by_date:
        sig_by_date[day].sort(key=lambda x: x[0], reverse=True)  # gap%降序
...
def main(data_path=None):
    ...
    测试期: 2022-06-01 ~ 数据最新日
    3组GAP阈值: 2.0% / 2.5% / 3.0% (验证信号密度与量子优势的关系)
    每组: walk-forward量子核SVM + 经典LogReg, 阈值扫描
    """
    log("=" * 70)
