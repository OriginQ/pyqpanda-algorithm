...
def plot_summary_panels(rows: list[dict[str, Any]], out_path: Path) -> None:
    ...
    panels = [
        ...
        ("probe_color_conn", "probe-site color correlation"),
    ...
    for ax, (key, title) in zip(axes.flatten(), panels):
        ...
        ax.set_xlabel("delta")
        ax.grid(True, alpha=0.28)
    fig.suptitle("SU(2) color-chain quantum diagnostics", y=0.995)
    fig.tight_layout()
    fig.savefig(out_path)
    plt.close(fig)
...
def main() -> None:
    ...
    parser.add_argument("--input", default=str(root / "outputs" / "summary.json"))
    parser.add_argument("--out", default=str(root / "outputs" / "figures"))
    ...
    plot_line(rows, "probe_color_conn", "connected color correlation", "left-right probe-site color correlation", out_dir / "probe_color_correlation.png")
    plot_line(rows, "fidelity_loss_to_previous", "1 - fidelity", "ground-state fidelity loss", out_dir / "fidelity_loss.png")
    plot_line(rows, "quench_delta_e", "Delta E after quench", "central color-singlet quench energy injection", out_dir / "quench_delta_energy.png")
