...
parameter-shift rule, which provides the wave-function Jacobian needed by the
...
# R(theta-pi/2)) / (2 sqrt(2))   (state-vector form).  The key is the gate
...
class VariationalAnsatz:
    ...
    init_state : ``ndarray``, optional\n
        Initial wave-function to be loaded before the parameterised gates are
    ...
    def get_statevector(self, theta: np.ndarray) -> np.ndarray:
        """Return the (normalised) state-vector produced by ``theta``.
    ...
    def get_jacobian(self, theta: np.ndarray) -> np.ndarray:
        """Return the wave-function Jacobian ``d|psi>/d theta_i``.
